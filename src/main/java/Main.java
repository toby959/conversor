...
// Cambio de color por consola  -- verde --
   ...
   /*
            ...
            Dotenv dotenv = Dotenv.load();
            String claveApi = dotenv.get("API_KEY");
            String direccion = "https://v6.exchangerate-api.com/v6/" + claveApi + "/latest/USD";
            ...
            while (opcion != 7) {
                ...
                if (opcion < 1 || opcion > 7) {
                    System.out.println("Opción inválida, seleccione un número entero entre 1 y 7 del menú.");
                    continue; // Volver al inicio del bucle sin procesar más
                }
                ...
                double cantConversion = scanner.nextDouble();
                if (cantConversion <= 0) {
                    System.out.println("El importe debe ser un número positivo.");
                    continue; // Volver al inicio del bucle sin procesar más
                }
...
//#############
                ...
                String jsonResponse = conexion.devolverRespuestaJson();
                ...
                if (jsonResponse == null) {
                    System.out.println("Error: La respuesta JSON es nula.");
                    return;
                }
//############
                MonedaOmdb monedaOmdb = crearMonedaOmdb(conexion.devolverRespuestaJson());

                if (monedaOmdb == null || monedaOmdb.conversion_rates() == null) {
                ...
                DecimalFormat df = new DecimalFormat("#,###.00");
                Double conversionRate = moneda.getConversiones().get(simboloFinal);

                if (conversionRate == null) {
                    System.out.println("Error: No hay tasa de conversión disponible para " + simboloFinal);
                    return;
                }

                double resultado = moneda.calcularConversion(cantConversion, conversionRate);
